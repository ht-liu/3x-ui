...
install() {
    bash <(curl -Ls https://github.com/ht-liu/3x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_menu() {
    ...
    wget -O /usr/bin/x-ui https://github.com/ht-liu/3x-ui/main/x-ui.sh
...
legacy_version() {
    ...
    install_command="bash <(curl -Ls "https://github.com/ht-liu/3x-ui/v$tag_version/install.sh") v$tag_version"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://github.com/ht-liu/3x-ui/master/install.sh)${plain}"
    echo ""
